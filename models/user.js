...
const findMany = ({ filters: { language } }) => {
  let sql = 'SELECT id, email, firstname, city, language FROM users';
  const sqlValues = [];
  if (language) {
    sql += ' WHERE language = ?';
...
const findOne = (id) => {
  return db
    .query('SELECT id, email, firstname, city, language FROM users WHERE id = ?', [id])
...
const create = ({ firstname, lastname, city, language, email, password}) =>{
  return hashPassword(password).then((hashedPassword) => {
    ...
    .then(([result]) => {
      const id = result.insertId;
      return { firstname, lastname, city, language, email }
    })
  })
}
